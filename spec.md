...
Tech Stack
Frontend uses React 19, Vite, React Router DOM, Pure Vanilla CSS (custom design system), Zustand, Axios, React Flow (@xyflow/react), Socket.IO client, and lucide-react icons. Backend uses Node.js, Express, MongoDB, Mongoose, JSON Web Tokens, BullMQ on Redis (via ioredis), Socket.IO, helmet, morgan, compression, express-validator, and bcryptjs. AI integration is done through the OpenRouter API and the Google Generative AI SDK, with a native 5-agent pipeline for agentic orchestration. OAuth and bot integrations cover Gmail, Slack, Discord, and Google Sheets. Sensitive credentials are encrypted at rest with an application-level key. DevOps includes automated CI/CD workflows using GitHub Actions.
...
The application is a pure React.js 19 Single Page Application (SPA) powered by Vite and React Router DOM. The root / page redirects authenticated users to the dashboard and unauthenticated users to login.
...
23242526272829303132
    ├── store/    │   ├── authStore.js    │   └── workflowStore.js    │    ├── lib/    │   ├── api.js    │   ├── socket.js    │   └── nodeCatalog.js    │    └── styles/ 
Expand
Backend Structure

151617181920212223242526
    ├── controllers/    │   ├── authController.js    │   ├── workflowController.js    │   ├── executionController.js    │   ├── integrationController.js    │   └── notificationController.js    │    ├── services/    │   ├── workflowService.js    │   ├── executionService.js    │   ├── orchestrationService.js 
Expand
Development Phases
